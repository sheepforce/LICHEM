                   ...
                   bool QMRegion,fstream& logFile)
{
  ...
  */
  ...
  if (QMRegion)
  {
    ...
    if ((RMSDiff <= QMMMOpts.QMOptTol) and (RMSForce <= QMMMOpts.QMRMSForceTol) and
...
void LICHEMNEB(vector<QMMMAtom>& QMMMData, QMMMSettings& QMMMOpts, int optCt,fstream& logFile)
{
  ...
  //End: Hatice
  {
    ...
    pathDone = PathConverged(QMMMData,oldQMMMData,forceStats,stepCt,QMMMOpts,1,logFile);
  }
  //Clean up files
  call.str("");
  call << "mkdir -p LICHM_CHARGES_LASTITER";
               ...
               int macroiter,fstream& logFile)
{
  ...
  fstream ofile,qmfile,initfile,hessfile,pathfile,gfile,p0file;//Generic file stream
  stringstream call; //Stream for system calls and reading/writing files
